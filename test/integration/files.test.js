...
describe('#Integration - Files - Files Structure', () => {
    ...
    // vai rodar do package.json que está na raiz, por isso iniciamos pegando da pasta test
    ...
    test('Repository class should have, read, update and delete methods', async () => {
        const myConfig = {
            ...config,
            layers: ['repository']
        }
    ...
    test('Service class should have the same signature of repository and call all its methods', async () => {
        ...
        getAllFunctionFromInstance(service).forEach(method => service[method].call(service, []))
